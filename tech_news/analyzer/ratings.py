...
def top_5_news():
    ...
    for news_item in news:
        rate = news_item["shares_count"]
        + news_item["comments_count"]
        get_tuple = (
            news_item["title"],
            news_item["url"], rate
        )
...
def top_5_categories():
    ...
    result = reduce(
        ...
        info, [])
    # counter = {category: result.count(category) for category in result}
    counter = list()
    for category in result:
        counter.append((category, result.count(category)))
    counter.sort(key=lambda item: item[0])  # Organiza em order alfabética
