...
def scrape_noticia(html_content):
    ...
    result["timestamp"] = selector.css("time#js-article-date::attr(datetime)").get()
    if selector.css("a.tec--author__info__link::text"):
        result["writer"] = selector.css("a.tec--author__info__link::text").get().strip()
    else:
        result["writer"] = None
    if selector.css("div.tec--toolbar__item::text"):
        result["shares_count"] = int(selector.css("div.tec--toolbar__item::text").get().strip()[0])
    else:
        result["shares_count"] = 0
    if selector.css("button#js-comments-btn::text"):
        result["comments_count"] = int(selector.css("button#js-comments-btn::attr(data-count)").get())
    else:
        result["comments_count"] = 0
    result["summary"] = "".join(selector.css("div.tec--article__body p:first-child *::text").getall())
    result["sources"] = []
